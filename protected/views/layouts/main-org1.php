        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav navbar-right">
               <?php  if(!Yii::app()->user->isGuest){ ?>
                ...
                <?php } ?>
               <?php  if(!Yii::app()->user->isGuest){ ?>
                    <li <?php if(Yii::app()->controller->id=='quotes' || Yii::app()->controller->id=='Quotes'){echo 'class="navbar-active"';}?> ><a href="<?php echo Yii::app()->createUrl('/quotes/index');?>"> <i class="fa fa-dollar"></i>  Quotes</a></li> <?php } ?>
               <?php  if(!Yii::app()->user->isGuest && 1==2){ ?>
                    <li><a href="<?php echo Yii::app()->createUrl('');?>">Orders</a></li> 
                <?php } ?>
               <?php  if(!Yii::app()->user->isGuest){ ?>
                    <li <?php if(Yii::app()->controller->id=='creditCardAuthorization' || Yii::app()->controller->id=='allForms'){echo 'class="navbar-active"';}?> ><a href="<?php echo Yii::app()->createUrl('/allForms/index');?>" class="dropdown-toggle" data-toggle="dropdown"> <i class="fa fa-clipboard"></i> Forms <b class="caret"></b></a>
                        <ul class="dropdown-menu">
                            <li>
                                <a href="?r=creditCardAuthorization/create">Credit Card Authorization</a>
                            </li>
                        </ul>
                    </li> 
               <?php } ?>
               <?php  if(!Yii::app()->user->isGuest){ ?>
                    <li <?php if(Yii::app()->controller->id=='DotTrackerUser' || Yii::app()->controller->id=='dotTrackerUser'){echo 'class="navbar-active"';}?> ><a href="<?php echo Yii::app()->createUrl('/DotTrackerUser/admin');?>" class="dropdown-toggle" data-toggle="dropdown"> <i class="fa fa-users"></i> Manage User</a></li> 
               <?php } ?>
               <?php  if(!Yii::app()->user->isGuest){ ?>
